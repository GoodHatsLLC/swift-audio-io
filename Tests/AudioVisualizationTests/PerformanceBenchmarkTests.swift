...
struct PerformanceBenchmarkTests {
  ...
  private let performanceLog = OSLog(subsystem: "AudioVisualizationTests", category: "Performance")
  ...
  func testRingBufferWritePerformance() async throws {
    ...
    os_signpost(.begin, log: performanceLog, name: "RingBuffer Write Test", signpostID: signpostID)
  ...
  func testConcurrentAudioProcessing() async throws {
    ...
    os_signpost(.begin, log: performanceLog, name: "Concurrent Processing", signpostID: signpostID)
    let startTime = CFAbsoluteTimeGetCurrent()
