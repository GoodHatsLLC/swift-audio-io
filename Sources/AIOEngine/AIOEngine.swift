...
public final class AIOEngine: Sendable {
  ...
  public var onRecordingInterruption: (@Sendable @MainActor (RecordingInterruption) async -> Void)?
  ...
  public func warm(configuration: RecordingConfiguration) throws {
    ...
    do {
      ...
      guard let tapConfiguration = configuration.tapConfiguration(bus: 0, input: inputFormat) else {
  ...
  public func scrubPlay(to time: TimeInterval) throws -> Playback? {
    if let initialInstance = state.playbackInstance {
      ...
      player
        .scheduleSegment(
          file,
          startingFrame: AVAudioFramePosition(framePosition),
